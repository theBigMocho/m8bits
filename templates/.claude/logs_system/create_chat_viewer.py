...
<head>
    ...
    <style>
        ...
        .chat-container {margin: auto; background-color: white; border: 1px solid #dddfe2; padding: 20px; border-radius: 8px; }
        .message { margin-bottom: 20px; display: flex; }
        .role { font-weight: bold; margin-bottom: 5px; }
        .user { justify-content: flex-end; }
        .user .role { color: #007bff; }
        .assistant .role { color: #28a745; }
        .message-content { padding: 12px; border-radius: 18px; line-height: 1.4; max-width: 75%; }
        .user .message-content { background-color: #e7f3ff; }
        .assistant .message-content { background-color: #f0f0f0; }
        ...
        .message-text pre { background-color: #f6f8fa; border-radius: 6px; padding: 16px; overflow-x: auto; margin: 1em 0; }
        .message-text code { background-color: rgba(175, 184, 193, 0.2); padding: 0.2em 0.4em; border-radius: 3px; font-size: 0.9em; }
        .message-text pre code { background-color: transparent; padding: 0; }
        .message-text p { margin: 0.5em 0; }
        .message-text blockquote { border-left: 4px solid #ddd; margin: 1em 0; padding-left: 1em; color: #666; }
        .images-container { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 10px; }
        img { max-width: 250px; display: block; border-radius: 15px; border: 1px solid #ddd; }
        .session-divider { text-align: center; color: #65676b; margin: 40px 15px 20px; font-weight: bold; font-size: 0.9em; }
    </style>
</head>
<body>
...
"""]
    ...
    if os.path.exists(log_file):
        ...
        for i in range(1, len(sessions), 3):
            ...
            session_images_html = ""
            session_window_start = session_dt_local - timedelta(minutes=5)
            session_window_end = session_dt_local + timedelta(minutes=5)
